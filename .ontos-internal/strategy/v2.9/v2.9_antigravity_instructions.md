...
You are tasked with implementing **v2.9** of Project Ontos, a "conscious memory system" for AI-native development teams. This release bridges V2 → V3 with four major features:

| Feature | Purpose | Status | Version |
|---------|---------|--------|---------|
| **Schema Versioning** | Forward compatibility for V3 migration | ✅ MERGED | v2.9.0 |
| **Curation Levels** | Lower adoption barrier with tiered validation | ✅ MERGED | v2.9.1 |
| **Deprecation Warnings** | Prepare users for V3 breaking changes | 🔄 NEXT | v2.9.2 |
| **install.py Bootstrap** | Single-file curl-based installation | 🔲 PENDING | v2.9.3 |

**Your primary reference:** `.ontos-internal/strategy/v2.9/v2.9_implementation_plan.md` (3,300+ lines, approved v1.4.0)
...
```
kernel/     → Mission, principles (rarely changes)
strategy/   → Roadmaps, plans (changes quarterly)
product/    → Features, specs (changes weekly)
atoms/      → Technical docs (changes daily)
logs/       → Session histories (append-only)
```
...
## 2. Current State (v2.9.1 stable)

> **Update 2025-12-23:** PR #31 (Schema Versioning) and PR #33 (Curation Levels) are MERGED.
> Version is now 2.9.1 with 390 tests passing.

### 2.1 What's Already Built

| Component | Status | Location |
|-----------|--------|----------|
| SessionContext (transactions) | ✅ Complete | `.ontos/scripts/ontos/core/context.py` |
| Unified CLI (ontos.py) | ✅ Complete | `ontos.py` |
| Frontmatter parsing | ✅ Complete | `.ontos/scripts/ontos/core/frontmatter.py` |
| Staleness detection | ✅ Complete | `.ontos/scripts/ontos/core/staleness.py` |
| OutputHandler (UI) | ✅ Complete | `.ontos/scripts/ontos/ui/output.py` |
| **Schema versioning** | ✅ **PR #31** | `.ontos/scripts/ontos/core/schema.py` |
| **Migration CLI** | ✅ **PR #31** | `.ontos/scripts/ontos_migrate_schema.py` |
| **Curation levels** | ✅ **PR #33** | `.ontos/scripts/ontos/core/curation.py` |
| **Scaffold CLI** | ✅ **PR #33** | `.ontos/scripts/ontos_scaffold.py` |
| **Stub CLI** | ✅ **PR #33** | `.ontos/scripts/ontos_stub.py` |
| **Promote CLI** | ✅ **PR #33** | `.ontos/scripts/ontos_promote.py` |
| 390 tests passing | ✅ Complete | `tests/` + `.ontos/scripts/tests/` |
...
.ontos/scripts/ontos/core/schema.py         # Schema versioning (PR #31)
.ontos/scripts/ontos/core/curation.py       # Curation levels (PR #33)
...
│                    ontos.py (CLI)                   │
├─────────────────────────────────────────────────────┤
│              ontos/ui/output.py (UI)                │
├─────────────────────────────────────────────────────┤
│    ontos/core/context.py (SessionContext)           │
│    ontos/core/frontmatter.py (Parsing)              │
│    ontos/core/staleness.py (Detection)              │
├─────────────────────────────────────────────────────┤
...
```
⚠️ CRITICAL: v2.x tools must use Python Standard Library ONLY (3.9+)
             NO pip install. NO third-party libraries.
```

This means:
- ❌ No `yaml` (PyYAML) - use custom parsing/serialization
- ❌ No `ruamel.yaml` - even for comment preservation
- ❌ No `requests` - use `urllib.request`
- ❌ No `click` - use `argparse` or manual parsing
- ✅ Use `json`, `pathlib`, `dataclasses`, `typing`, `re`, etc.
...
```
PR #31: Schema Versioning (v2.9.0)     ✅ MERGED
                                          │
PR #33: Curation Levels (v2.9.1)       ✅ MERGED
                                          │
PR #34: Deprecation Warnings (v2.9.2)  🔄 NEXT  ← YOU ARE HERE
                                          │
PR #35: install.py Bootstrap (v2.9.3)  🔲 PENDING
                                          │
PR #36: Documentation & Release        🔲 PENDING
```

### 4.2 PR #31: Schema Versioning ✅ MERGED

> **Status:** MERGED on 2025-12-22
>
> This PR is complete. The actual implementation differs slightly from the original plan.
> See the "Actual API" section below for the real interface to use in PR #32+.

**Goal:** Add `ontos_schema` field and migration tooling

**Files Added:**
```
.ontos/scripts/ontos/core/schema.py       # 421 lines, 8 functions
.ontos/scripts/ontos_migrate_schema.py    # 325 lines, CLI script
.ontos/scripts/tests/test_schema.py       # 307 lines, 42 tests
```

**Actual API (Use This in PR #32+):**

```python
# Import from schema.py
from ontos.core.schema import (
    # Functions
    parse_version,           # (str) -> Tuple[int, int]
    detect_schema_version,   # (dict) -> str
    check_compatibility,     # (doc_version, tool_version) -> SchemaCheckResult
    validate_frontmatter,    # (dict, Optional[str]) -> Tuple[bool, List[str]]
    serialize_frontmatter,   # (dict) -> str
    add_schema_to_frontmatter,  # (dict, Optional[str]) -> dict
    get_schema_info,         # (str) -> Optional[dict]

    # Constants
    CURRENT_SCHEMA_VERSION,  # "2.2"
    MIN_READABLE_SCHEMA,     # "1.0"
    MAX_READABLE_SCHEMA,     # "2.2"
    SCHEMA_DEFINITIONS,      # dict of all schema versions

    # Classes
    SchemaCompatibility,     # Enum: COMPATIBLE, READ_ONLY, INCOMPATIBLE
    SchemaCheckResult,       # Dataclass with compatibility, versions, message
)
```
...
|--------|--------------|----------------------|
| `check_compatibility()` return | `SchemaCompatibility` | `SchemaCheckResult` (richer) |
| Compatibility values | `UPGRADEABLE` | `READ_ONLY` |
| `analyze_document()` | Planned | Not implemented (not needed) |
| `migrate_frontmatter()` | Planned | Handled in migration script |
...
### 4.3 PR #33: Curation Levels ✅ MERGED

> **Status:** MERGED on 2025-12-22 as v2.9.1
>
> This PR is complete. See the "Actual API" section below for the real interface.

**Goal:** Tiered validation (L0 Scaffold → L1 Stub → L2 Full)

**Files Added:**
```
.ontos/scripts/ontos/core/curation.py     # ~490 lines, level detection/validation
.ontos/scripts/ontos_scaffold.py          # ~230 lines, generate L0 scaffolds
.ontos/scripts/ontos_stub.py              # ~200 lines, create L1 stubs
.ontos/scripts/ontos_promote.py           # ~440 lines, interactive promotion
.ontos/scripts/tests/test_curation.py     # ~350 lines, 42+ tests
.ontos/scripts/tests/test_promote.py      # ~130 lines, 16+ tests
```

**Actual API (Use This in PR #34+):**

```python
from ontos.core.curation import (
    # Enums/Classes
    CurationLevel,        # IntEnum: SCAFFOLD=0, STUB=1, FULL=2
    CurationInfo,         # Dataclass with level, issues, promotable

    # Level detection
    detect_curation_level,     # (dict) -> CurationLevel
    validate_at_level,         # (dict, CurationLevel) -> (bool, List[str])
    get_curation_info,         # (dict) -> CurationInfo

    # Creation
    create_scaffold,           # (Path, Optional[str]) -> dict
    create_stub,               # (id, type, goal, depends_on) -> dict
    promote_to_full,           # (dict, depends_on, concepts) -> (dict, summary)

    # Helpers
    infer_type_from_path,      # (Path) -> str
    level_marker,              # (CurationLevel) -> str like "[L0]"
    load_ontosignore,          # (Path) -> List[str]
)
```

**CLI Commands:**
- `python3 ontos.py scaffold` — Generate L0 scaffolds (`--apply` to execute)
- `python3 ontos.py stub` — Create L1 stubs
...
- Curation statistics in `python3 ontos.py maintain`

### 4.4 PR #34: Deprecation Warnings (v2.9.2) 🔄 NEXT
...
### 4.5 PR #35: install.py Bootstrap (v2.9.3)
