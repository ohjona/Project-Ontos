...
>
> Push your code — sessions are archived, old logs are consolidated, nothing is left behind. You focus on building.
...
|------|-----------|----------|----------|
| **pre-commit** | Consolidate if over threshold → auto-stage | — | — |
| **pre-push** | Auto-archive session | Block until archived | Warn if unarchived |
| **activation** | — | Remind if consolidation needed | — |
...
2. Pre-push hook runs consolidation → moves files
...
5. Consolidation runs → moves files, updates decision_history.md
...
╠════════════════════════════════════════════════════════════════╣
   ...
   [2] Prompted (recommended for teams) [DEFAULT]
       ...
       • Agent reminds you to consolidate
...
def main() -> int:
    ...
    if success:
        ...
        print("   ✓ No logs old enough to consolidate")
        ...
        if os.path.exists(pre_commit_dst):
            ...
            with open(pre_commit_src, 'r') as src, open(pre_commit_dst, 'r') as dst:
                if src.read() != dst.read():
                    ...
                    print(f"   ℹ Backed up existing pre-commit to {backup}")
...
4. **Check consolidation status (prompted/advisory modes only):**
   ...
   - If count > LOG_RETENTION_COUNT (default: 15):
     ```
     print("⚠️ {count} active logs (threshold: {limit}). Run 'Consolidate Ontos' to archive old sessions.")
     ```
...
## 12. Open Questions for Architectural Review

This section captures architectural decisions that would benefit from additional perspectives. Each question includes options considered and a preliminary recommendation.

---

### Q1: Threshold Logic — Count vs Age Mismatch

**Context:** The current design triggers consolidation based on COUNT (`log_count > 15`) but consolidates based on AGE (`--days 30`). These can conflict.

**Scenario:** User has 20 logs, all from the past week. Hook triggers (count > 15), but consolidation finds nothing older than 30 days. Output: "No logs old enough to consolidate" — potentially confusing.

**Options:**

| Option | Trigger | Consolidate | Pros | Cons |
|--------|---------|-------------|------|------|
| A | Count > threshold | Oldest N logs | Consistent mental model | May archive recent important logs |
| B | Age > threshold | Age > threshold | Only archives stale content | Count can grow unbounded |
| C | Count > threshold AND has old logs | Age > threshold | Best of both | More complex logic |

**Preliminary Recommendation:** Option C — Trigger on count, but only if there ARE logs old enough to consolidate. This prevents confusing "nothing to do" messages.
...
**Question for reviewers:** Is Option C over-engineered? Would Option B (age-only) be simpler and sufficient?

---

### Q2: Repeated Consolidation on Multiple Commits

**Context:** If a user commits 5 times in one session, the pre-commit hook runs 5 times. After the first consolidation, subsequent runs find nothing.

**Options:**

| Option | Approach | Pros | Cons |
|--------|----------|------|------|
| A | Silent when nothing to consolidate | Clean output | User doesn't know hook ran |
| B | Add "last consolidated" timestamp | Prevents redundant runs | More state to manage |
| C | Keep current behavior | Simple | Noisy output |

**Preliminary Recommendation:** Option A — Complete silence when there's nothing to consolidate. Only print when consolidation actually does work.

**Question for reviewers:** Is silent failure acceptable, or should there be a verbose/debug flag?

---

### Q3: Unicode Box Drawing in Terminal

**Context:** The promise messaging UI uses Unicode box-drawing characters (`╔`, `║`, `╚`). These may not render correctly in Windows CMD, some SSH sessions, or CI logs.

**Options:**

| Option | Approach | Pros | Cons |
|--------|----------|------|------|
| A | Keep Unicode | Looks polished in modern terminals | Breaks in legacy terminals |
| B | ASCII-only (`=`, `|`, `-`) | Universal compatibility | Less visually distinct |
| C | Auto-detect and fallback | Best of both | More code complexity |

**Preliminary Recommendation:** Option B — Use ASCII-only. Ontos targets developers who may SSH from various environments. Reliability over polish.

**Question for reviewers:** Is the visual polish of Unicode worth the compatibility risk? Or is Option C (auto-detect) worth the complexity?

---

### Q4: Existing Pre-commit Hook Conflicts

**Context:** Many projects already have pre-commit hooks (eslint, prettier, husky, pre-commit framework). Current plan backs up and replaces, which breaks existing workflows.

**Options:**

| Option | Approach | Pros | Cons |
|--------|----------|------|------|
| A | Replace with backup | Simple implementation | Breaks existing hooks |
| B | Append Ontos call to existing hook | Preserves user hooks | Complex parsing |
| C | Warn and skip if non-Ontos hook exists | Safe, non-destructive | User must manually integrate |
| D | Support pre-commit framework (.pre-commit-config.yaml) | Industry standard | Additional dependency |

**Preliminary Recommendation:** Option C — Detect non-Ontos hooks, warn, provide manual integration instructions. Don't silently break user workflows.

```python
if os.path.exists(pre_commit_dst):
    with open(pre_commit_dst, 'r') as f:
        if 'ontos' not in f.read().lower():
            print("   ⚠ Existing pre-commit hook detected")
            print("   Add this line to your hook: python3 .ontos/scripts/ontos_pre_commit_check.py")
            return  # Don't overwrite
```

**Question for reviewers:** Should we support the `pre-commit` framework (Option D) as a first-class integration path? It's widely used but adds external dependency awareness.

---

### Q5: Agent Reminder Enforcement

**Context:** In prompted mode, the plan adds text to Agent Instructions saying "check consolidation status and remind user." This relies on agents reading and following prose — not enforced by code.

**Options:**

| Option | Approach | Pros | Cons |
|--------|----------|------|------|
| A | Documentation only (current) | Simple | Unreliable |
| B | Add `ontos_check.py --consolidation` for agents to call | Scriptable | Agents must know to call it |
| C | Add warning to context map generation | Always visible | Clutters context map |
| D | Accept as advisory, document limitation | Honest | Doesn't solve the problem |

**Preliminary Recommendation:** Option D for v2.5 — Be honest that prompted mode reminders are advisory, not enforced. Consider Option B for v2.6.

**Question for reviewers:** Is it acceptable for prompted mode consolidation reminders to be "best effort"? Or should v2.5 include a scriptable check (Option B)?

---

### Q6: File Changes Summary — Keep or Remove?

**Context:** Section 6 estimates lines changed per file. These estimates will be wrong after implementation.

**Options:**

| Option | Approach |
|--------|----------|
| A | Keep estimates | Gives rough scope sense |
| B | Remove estimates, keep file list | Accurate without noise |
| C | Remove section entirely | Implementation plan, not project plan |

**Preliminary Recommendation:** Option B — Keep the file list (useful for implementation), remove line estimates (always wrong).

**Question for reviewers:** Any value in keeping line estimates for scoping purposes?

---

## 13. Reviewer Responses

*Space for other models/reviewers to provide their perspectives on the open questions.*
...
- [ ] Open questions resolved (Section 12)
